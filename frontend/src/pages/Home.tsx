...
import {
    ...
    Map as MapIcon, ChevronRight, Activity, Download, BarChart3,
...
function StatBox({ metric, index }: { metric: any, index: number }) {
    ...
    return (
        ...
        >
            ...
            <span className="stat-number block mb-2">{count}{metric.suffix}</span>
            <h3 className="text-white/90 font-medium mb-2">{metric.label}</h3>
            <p className="text-white/50 text-sm leading-relaxed">{metric.description}</p>
        </div>
    )
}
    ...
    return (
        <div className="min-h-screen bg-nasa-dark selection:bg-water-500/30">
            ...
            <section className="relative min-h-screen flex items-center pt-24 pb-12 overflow-hidden particles-bg section-dark">
                ...
                <div className="absolute bottom-1/4 right-1/4 w-[35rem] h-[35rem] bg-eco-600/20 rounded-full mix-blend-screen filter blur-[100px] animate-pulse-slow pointer-events-none" style={{ animationDelay: '2s' }} />

                <div className="max-w-7xl mx-auto px-6 relative z-10 w-full grid lg:grid-cols-2 gap-16 items-center">
                    <div ref={heroRef} className={`${heroVisible ? 'animate-slide-up' : 'opacity-0'}`}>
                        ...
                        <p className="text-xl text-white/70 leading-relaxed mb-10 max-w-xl">
                            Sistema geoespacial de micro-represas IoT para proteger la provincia de Córdoba de los extremos climáticos de El Niño y La Niña.
                        </p>
                        ...
                        <div className="mt-12 flex items-center gap-6 border-t border-white/10 pt-8">
                            <div className="flex -space-x-4">
                                {[1, 2, 3, 4].map(i => (
                                    <div key={i} className="w-10 h-10 rounded-full border-2 border-nasa-dark bg-white/10 backdrop-blur-md flex items-center justify-center text-xs font-bold text-white/70">
                                        U{i}
                                    </div>
                                ))}
                            </div>
                            <p className="text-sm text-white/50">Investigación abierta liderada por estudiantes del IPET 66</p>
                        </div>
                    </div>
                        ...
                        <div className="glass-card relative z-10 p-8 border-water-500/30 animate-float">
                            <div className="flex items-center justify-between mb-8 pb-6 border-b border-white/10">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-white">Monitoreo Satelital</h3>
                                        <p className="text-xs text-white/50">Estado de Cuencas en Tiempo Real</p>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-4">
                                {cuencasData.slice(0, 3).map((cuenca) => (
                                    <div key={cuenca.id} className="bg-white/5 rounded-xl p-4 flex items-center justify-between">
                                        <div>
                                            <h4 className="text-white text-sm font-medium">{cuenca.name}</h4>
                                            <p className="text-xs text-white/40 mt-1">Nivel: Óptimo</p>
                                        </div>
                                        <div className="w-24 h-2 bg-white/10 rounded-full overflow-hidden">
                                            <div className="w-[85%] h-full bg-water-400 rounded-full shadow-[0_0_10px_#48d6ff]" />
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            <section className="py-32 relative section-dark">
                <div className="max-w-7xl mx-auto px-6">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {/* Tech cards */}
                        {[
                            { title: 'IoT & Firmware', tags: techStack.filter(t => t.category === 'hardware' || t.category === 'firmware'), icon: Droplets, color: 'text-blue-400', bg: 'bg-blue-400/10' },
                            { title: 'Inteligencia Geoespacial', tags: techStack.filter(t => t.category === 'geo'), icon: MapIcon, color: 'text-eco-400', bg: 'bg-eco-400/10' },
                            { title: 'Software Ecosystem', tags: techStack.filter(t => t.category === 'programming' || t.category === 'tools'), icon: Globe, color: 'text-purple-400', bg: 'bg-purple-400/10' },
                        ].map((col, i) => (
                            <div key={i} className="glass-card p-8 hover-lift border-t border-t-white/10 relative overflow-hidden">
                                <div className={`absolute top-0 right-0 w-32 h-32 rounded-bl-full ${col.bg} blur-2xl opacity-50`} />
                                <div className={`w-12 h-12 rounded-xl ${col.bg} flex items-center justify-center mb-6`}>
                                    <col.icon className={`w-6 h-6 ${col.color}`} />
                                </div>
                                <h3 className="text-xl font-display font-bold text-white mb-6">{col.title}</h3>
                                <div className="flex flex-wrap gap-2">
                                    {col.tags.map(tag => (
                                        <span key={tag.name} className="px-3 py-1.5 rounded-md bg-white/5 border border-white/10 text-white/70 text-sm font-medium hover:bg-white/10 transition-colors">
                                            {tag.name}
                                        </span>
                                    ))}
                                </div>
            ...
            <section id="demo" className="py-32 relative section-alt border-y border-white/[0.05]">
                        ...
                        <div className="grid lg:grid-cols-2 gap-0 divide-y lg:divide-y-0 lg:divide-x divide-white/10">
                            ...
                            <div className="p-10 space-y-8">
                                <div>
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        {[
                                            { id: 'normal', label: 'Neutral', bg: 'bg-white/5 hover:bg-white/10', border: 'border-white/10' },
                                            { id: 'nino', label: 'El Niño', bg: 'bg-red-500/10 hover:bg-red-500/20', border: 'border-red-500/30' },
                                            { id: 'nina', label: 'La Niña', bg: 'bg-water-500/10 hover:bg-water-500/20', border: 'border-water-500/30' },
                                        ].map((mode) => (
                                            <button
                                                key={mode.id}
                                                onClick={() => setPhenomenon(mode.id as any)}
                                                className={`py-3 px-4 rounded-xl text-sm font-medium transition-all border ${phenomenon === mode.id ? mode.border + ' ' + mode.bg.split(' ')[0] + ' text-white shadow-lg' : 'border-white/5 bg-white/5 text-white/50'
                                                    }`}
                                            >
                                                {mode.label}
                                            </button>
                                ...
                                <div>
                                    <div className="flex justify-between mb-4">
                                        ...
                                        <span className="text-water-400 font-mono font-bold">{precipitation} mm</span>
                                    </div>
                                    <input
                                        type="range" min="0" max="250" value={precipitation}
                            ...
                            <div className="p-10 flex flex-col justify-center items-center relative overflow-hidden">
                                ...
                                <div className="bg-white/5 border border-white/10 rounded-xl p-6 text-center max-w-sm w-full z-10 backdrop-blur-md">
                                    <p className="text-sm text-white/40 uppercase font-semibold mb-2">Recomendación del Sistema</p>
            ...
            <section className="py-24 relative overflow-hidden section-dark">
                ...
                <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
                    ...
                    <p className="text-xl text-white/70 mb-10 max-w-2xl mx-auto leading-relaxed">
                        Descubre nuestro estudio comparativo detallado y la documentación de la arquitectura tecnológica.
                    </p>
